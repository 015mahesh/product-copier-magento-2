 ...
 */
class MassReset extends ReportController
{
    /**
     * Mass Action Filter
     *
    ...
    public function execute()
    {
        ...
        foreach ($collectionFilter as $item) {
            ...
            if ($count >= $maxRecord || $item->getId() == $lastItemId) {
                $reportModel->resetMultiReports($collectionArr);
                $collectionArr = [];
                $count = 0;
            }
        }

        $this->messageManager->addSuccess(__('A total of %1 record(s) have been reset.', $collectionSize));
